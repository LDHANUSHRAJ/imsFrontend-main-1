...
import { Plus, Check, X, Eye, Briefcase, Building2, MapPin } from 'lucide-react';
import { fetchJobs, fetchJobsByRecruiter, updateJobStatus, createJob } from '../../services/job.service';
    ...
    const loadJobs = async () => {
        ...
        try {
            ...
            if (user?.role === 'RECRUITER') {
                data = await fetchJobsByRecruiter();
            } else {
                ...
                {isLoading ? (
                    ...
                    jobs.map((job) => (
                        <TableRow key={job.id || job._id}>
                            <TableCell>
                                <div>
                                    ...
                                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter flex items-center gap-1 mt-0.5">
                                        <MapPin size={10} /> {job.location || job.address || 'Remote'}
                                    </div>
                                </div>
                            </TableCell>
                            ...
                            <TableCell className="font-bold text-[#3B82F6]">₹{job.remuneration || job.stipend || '0'}/mo</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                                <div className="flex items-center gap-3">
                                    ...
                                    {(user?.role === 'IC' || user?.role === 'HOD') && job.status === 'PENDING' && (
                                        <>
                                            <button onClick={() => handleAction(job.id || job._id, 'APPROVED')} className="text-emerald-500 hover:bg-emerald-50 p-1 rounded transition-colors" title="Approve & Publish">
                                                <Check size={18} strokeWidth={3} />
                                            </button>
                                            <button onClick={() => handleAction(job.id || job._id, 'REJECTED')} className="text-rose-500 hover:bg-rose-50 p-1 rounded transition-colors" title="Reject">
